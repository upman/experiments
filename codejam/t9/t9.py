...
a=int(f.readline()) #to read the number of test cases.
t9dict={'\n':'\n',' ':'0','a':'2','b':'22',\
...
for line in f :
    count+=1
    j=line[0] #Store the previous entry to check if a space has to be given
    fo.write("Case #"+str(count)+": ")
    for i in line:
        if(t9dict[i][-1]==t9dict[j][-1]): #add a space if previous letter is same as the current one
            fo.write(' '+t9dict[i])       
